...
fprintf('Delay times is: %d seconds \n', opt.delay);
...
if isempty(collection_) || opt.png~=png_ || opt.pdf ~= pdf_ || ~strcmp(opt.memtype,memtype_) || opt.delay~=delay_
    ...
    save(fullfile(homedir,'..','per_region_fraction_collection.mat'),'collection');
    colormap('jet')
    ...
    if opt.png
        ...
        print(fh, fullfile(homedir,'..','plots','per_region_fraction.png'), '-dpng', '-r300' );   %save file as PNG w/ 300dpi
    end
    if opt.pdf
        set(gcf, 'PaperSize', [12 40])    % Same, but for PDF output
        print(gcf, fullfile(homedir,'..','plots','per_region_fraction.pdf'), '-dpdf', '-r300' );   %save file as PDF w/ 300dpi
    end
