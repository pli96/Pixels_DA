...
function [fh,bh]=conn_prob_bars_sensory_zha(sig,pair,opt)
arguments
    sig (1,1) struct
    pair (1,1) struct
...
sess_sig_type=sig.sensoryResp(sig_same(:,opt.dist),:);
sess_pair_type=pair.sensoryResp(pair_same(:,opt.dist),:);
same_stats=get_ratio(sess_sig_type,sess_pair_type);

%h2l
sess_sig_type=sig.sensoryResp(sig_h2l(:,opt.dist),:);
sess_pair_type=pair.sensoryResp(pair_h2l(:,opt.dist),:);
h2l_stats=get_ratio(sess_sig_type,sess_pair_type);

%l2h
sess_sig_type=sig.sensoryResp(sig_l2h(:,opt.dist),:);
sess_pair_type=pair.sensoryResp(pair_l2h(:,opt.dist),:);
l2h_stats=get_ratio(sess_sig_type,sess_pair_type);

fprintf('Sensory Resp\n');
