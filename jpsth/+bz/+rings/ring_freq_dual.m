homedir='/OceanStor100D/home/lichengyu_lab/lipy/neuropixel/dual_task';
codedir='/OceanStor100D/home/lichengyu_lab/lipy/neuropixel/code';

%% ring freq
load(fullfile(homedir,'xcorr','rings_bz.mat'))
load(fullfile(homedir,'session_list.mat'))
for sess=sessid    
    [spkTS,spkID]=spk(session{sess},homedir,sess,codedir);
    %     load(fullfile(homedir,'DataSum',session{sess},'FT_SPIKE.mat'))
    for rsize=4
        sums=cell(0);
        if isempty(rings{sess,rsize-2})
            continue
        end
...
%% function
function [spkTS,spkID]=spk(Sessionfolder,HomePath,sessionIdx,codepath)
