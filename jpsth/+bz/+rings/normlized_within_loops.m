...
% homedir = ephys.util.getHomedir('dtype',opt.type,'type','sums'); % ~/xcorr directory
if ~exist('ring_meta','var')
%     ring_meta=bz.rings.get_ring_meta('shufrpt',100,'type','neupix','criteria','Learning','delay',6,'congru',true,'nonmem',true,'loadfile',true);
...
plot_two(jwithin.congru.within,...
    ...
    'ylim',[-0.2,0.2]);
% exportgraphics(gcf,fullfile(homedir,'..','plots','loops_per_region_norm.pdf'))
exportgraphics(gcf,'loops_per_region_norm.pdf')
ylim([0.01,100])
set(gca(),'YScale','log')
yline(1.96,'--k')
exportgraphics(gcf,'loops_per_region_norm_log.pdf')
% exportgraphics(gcf,fullfile(homedir,'..','plots','loops_per_region_norm_log.pdf'))
...
for ri=1:numel(ureg)
    if ~ismember(ureg(ri),BGS) % 用binofit是因为shuffle做了多次，所以要取平均
        [phat,pci]=binofit(0,numel(shuf));  % 如果该脑区不存在于shuffle数据的ring所在脑区，那就是0对
        phat=0;
        pci=[0,0];
    else
        [phat,pci]=binofit(BS(strcmp(BGS,ureg(ri))),numel(shuf)); % BS(strcmp(BGS,ureg(ri))) 获得在shuffle数据中该脑区存在于几个ring中，numel(shuf) 即 shuffle数据获得的ring的总数
    end
    normw(ri,2)=phat;
