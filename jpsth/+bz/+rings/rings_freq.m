...
load(fullfile(homedir,'rings_bz.mat'),'rings');
%loop entry
if sessidx>size(rings,1) || isempty(rings{sessidx,rsize-2})
    if isunix, quit(0);else, return;end
end
sess_rings=rings{sessidx,rsize-2};
[spkID,spkTS,trials,suids,folder]=ephys.getSPKID_TS(sessidx,'type',opt.type,'criteria','any');
...
for ring_id=rids
    ...
    % save(fullfile(homedir,'ring','spkTS.mat'),'ts_id','-v7.3');
...
if size(sums,1)>0
    save(fullfile(homedir,'ring',sprintf('ring_stats_%d_%d_%d_%d.mat',rsize,sessidx,min(rids),size(sums,1))),'sums','-v7.3');
end
end
