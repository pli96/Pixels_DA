...
%     incosel=stats.incongru.maxiter(:,1)==0 & stats.incongru.diff_reg(:,opt.treedepth);
    congsel=fullstats.congru.diff_reg(:,opt.treedepth);
    nonmsel=fullstats.nonmem.diff_reg(:,opt.treedepth);
    incosel=fullstats.incongru.diff_reg(:,opt.treedepth);
else
...
%     incosel=stats.incongru.maxiter(:,1)==0 & stats.incongru.same_reg(:,opt.treedepth);
    congsel=fullstats.congru.same_reg(:,opt.treedepth);
    nonmsel=fullstats.nonmem.same_reg(:,opt.treedepth);
    incosel=fullstats.incongru.same_reg(:,opt.treedepth);
end

if strcmp(opt.dir,'from'), pos=1;else, pos=2;end
congreg=cellfun(@(x,idx) x(idx,:),fullstats.congru.reg(congsel,:),opt.treedepth,'UniformOutput',false);
nonmreg=cellfun(@(x) x{pos}{opt.treedepth},fullstats.nonmem.reg(nonmsel,:),'UniformOutput',false);
incoreg=cellfun(@(x) x{pos}{opt.treedepth},fullstats.incongru.reg(incosel,:),'UniformOutput',false);
...
congrudata=fliplr(fullstats.congru.coeff(congsel,2:end))*100;
nonmdata=fliplr(fullstats.nonmem.coeff(nonmsel,2:end))*100;
incodata=fliplr(fullstats.incongru.coeff(incosel,2:end))*100;
