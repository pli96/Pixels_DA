...
if strcmp(opt.correct_error,'any')
    if strcmp(opt.type,'neupix') || strcmp(opt.type,'MY') || strcmp(opt.type,'dual_task') || strcmp(opt.type,'ODR2AFC')
    ...
    if strcmp(opt.correct_error,'correct')
        ...
        elseif strcmp(opt.type,'AIOPTO')
            ...
            target_trials=find(all(trials(:,end-1:end)>0,2));
        end
...
%         if tt==28, keyboard;end
        ...
        tonset=trials(tt,1)+30000*opt.starts;
        toffset=trials(tt,1)+30000*opt.ends;
