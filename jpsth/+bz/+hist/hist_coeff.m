...
homedir=ephys.util.getHomedir('type','sums', 'dtype', opt.type);
if ~strcmp(opt.correct_error,'any')
    save(fullfile(homedir, 'bzdata', sprintf('%s_stp_%s_%d_%d%s%s.mat',...
        opt.prefix,opt.correct_error,sess,opt.tsbin_size,laser_suffix,epoch_suffix)),...
        'postspk_out','fc_out','sess_suids','sess');
else
    save(fullfile(homedir, 'bzdata', sprintf('%s_stp_%s_%d_%d%s%s.mat',...
        opt.prefix,opt.correct_error,sess,opt.tsbin_size,laser_suffix,epoch_suffix)),...
        'postspk_out','sess_suids','sess');
end
