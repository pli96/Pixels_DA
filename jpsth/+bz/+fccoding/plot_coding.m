...
% {FWD/RWD,H2L/L2H/Local, S1/S2*correct/error*3s/6s} - zxx
% congru, incongru, nonmem

if opt.plot_trial_frac
    [metas,stats,~]=bz.fccoding.get_fc_coding('type','dual_task');
    ...
    hold on
    swarmchart(ones(nnz(congrus1+congrus2),1),[stats(congrus1,5);stats(congrus2,6)],1,[0.8,0.8,0.8],'o','MarkerFaceAlpha',0.2,'MarkerFaceColor','r','MarkerEdgeColor','none')
...
if opt.plot_coding_idx
    [metas,stats,~]=bz.fccoding.get_fc_coding('no_shift',true);
