...
%Fieldtrip routine
[~,~,trials,~,~,FT_SPIKE]=ephys.getSPKID_TS(fidx,'keep_trial',true,'suids',unique(suids),'only_delay',true,'criteria',opt.criteria,'type',opt.type,'delay',opt.delay,'starts',opt.starts,'ends',opt.ends);
%TOD: apply trial number criteria
onesess=struct();
onesess.trials=trials; % col 8=delay
onesess.fidx=fidx;
onesess.fc=[];
...
save(fullfile(homedir,'fcdata',sprintf('fc_coding_%d.mat',fidx)),'onesess','-v7.3')
end
