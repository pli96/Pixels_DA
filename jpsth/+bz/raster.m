...
%Fieldtrip routine
[spkID,spkTS,trials,~,~]=ephys.getSPKID_TS(fidx,'type',opt.type,'criteria',opt.criteria);
...
sps=30000;
cfg=struct();
cfg.trl=[trials(:,1)-3*sps,trials(:,1)+11*sps,zeros(size(trials,1),1)-3*sps,trials]; % modify to suit ITI
cfg.trlunit='timestamps';
cfg.timestampspersecond=sps;
FT_SPIKE=ft_spike_maketrials(cfg,FT_SPIKE);
%%%%%%%
pref_trial_sel=find(trials(:,end-1)>0 & trials(:,end)>0 & trials(:,8)==opt.delay & trials(:,5)==prefsamp);
if ~isfield(opt,'tsel')
