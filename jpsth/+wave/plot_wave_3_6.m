...
for sess=30
    while true
        fs=sprintf('s%d',sess);
        com_map_3=wave.get_com_map('onepath',fullfile(homedir,ephys.sessid2path(sess,'type',opt.type,'criteria',opt.criteria)),'curve',true,'rnd_half',false,'delay',3,'type',opt.type,'criteria',opt.criteria);
        com_map_6=wave.get_com_map('onepath',fullfile(homedir,ephys.sessid2path(sess,'type',opt.type,'criteria',opt.criteria)),'curve',true,'rnd_half',false,'delay',6,'type',opt.type,'criteria',opt.criteria,'partial','early3in6');
...
%         exportgraphics(fh,sprintf('wave_half_half_%d.png',sess));
        keyboard()        
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('wave_3s_6s_%d.pdf',sess)));
%         close all
%         waitfor(fh)
