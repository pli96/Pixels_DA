...
arguments
    ...
    opt.plot_laser_on_off (1,1) logical = false
    opt.type (1,:) char {mustBeMember(opt.type,{'neupix','AIOPTO','MY','ODR2AFC','dual_task','VDPAP'})}='neupix'
    opt.criteria (1,:) char {mustBeMember(opt.criteria,{'Learning','WT','any'})} = 'WT'
end

persistent com_str_early3 com_str_late3 com_str_3 com_str_6 com_str_laseron com_str_laseroff
...
if opt.plot_3_6early
    if isempty(com_str_early3) || isempty(com_str_3)
        com_str_early3=wave.get_com_map('partial','early3in6','type',opt.type,'criteria',opt.criteria,'delay',6);
        com_str_3=wave.get_com_map('delay',3,'type',opt.type,'criteria',opt.criteria);
    end
    ...
    exportgraphics(fh,fullfile(homedir,'..','plots','TCOM_corr_3s_6early3.pdf','ContentType','vector'));
end

%% 3 and 6late3
if opt.plot_3_6late
    if isempty(com_str_late3) || isempty(com_str_3)
        com_str_late3=wave.get_com_map('partial','late3in6','type',opt.type,'criteria',opt.criteria,'delay',6);
        com_str_3=wave.get_com_map('delay',3,'type',opt.type,'criteria',opt.criteria);
    end
...
%     keyboard()
    exportgraphics(fh,fullfile(homedir,'..','plots','TCOM_corr_3s_6late3.pdf','ContentType','vector'));
end
...
%     keyboard()
    exportgraphics(fh,fullfile(homedir,'..','plots','TCOM_corr_3s_6s.pdf','ContentType','vector'));
end
%% 6 and 6early3
...
%     keyboard()
    exportgraphics(fh,fullfile(homedir,'..','plots','TCOM_corr_6searly_6slate.pdf','ContentType','vector'));
end

%% laser on and laser off
if opt.plot_laser_on_off
    if isempty(com_str_laseron) || isempty(com_str_laseroff)
        com_str_laseron=wave.get_com_map('partial','after','type',opt.type,'criteria',opt.criteria,'delay',6,'laser','on');
        com_str_laseroff=wave.get_com_map('partial','after','delay',6,'type',opt.type,'criteria',opt.criteria,'laser','off');
    end
    allsess=intersect(fieldnames(com_str_laseron),fieldnames(com_str_laseroff));
    corrmat=[];
    for sess=reshape(allsess,1,[])
        for pref=["s1","s2"]
            allcid=intersect(cell2mat(com_str_laseron.(sess{1}).(pref).keys),cell2mat(com_str_laseroff.(sess{1}).(pref).keys));
            for onecid=allcid(:).'
                corrmat=[corrmat;...
                    com_str_laseron.(sess{1}).(pref)(onecid),com_str_laseroff.(sess{1}).(pref)(onecid)];
            end
        end
    end
    corrmat=rmmissing(corrmat);
    [r3e,p3e]=corr(corrmat(:,1),corrmat(:,2));
    % rci3e=bootci(500,@(x) corr(x(:,1),x(:,2)),corrmat);
    binavg=[];
    [~,~,kbin]=histcounts(corrmat(:,2),0:2:12);
    ...
    scatter(corrmat(:,2)./4,corrmat(:,1)./4,4,'k','filled','o','MarkerEdgeColor','none','MarkerFaceAlpha',0.2);
    text(max(xlim()),max(ylim()),sprintf('r=%.3f,p=%.3f,n=%d',r3e,p3e,size(corrmat,1)),'HorizontalAlignment','right','VerticalAlignment','top');
    phi=plot([0,1.5],[0,1.5],'--r','LineWidth',1);
    ylabel('TCOM in laser on trials (s)');
    xlabel('TCOM in laser off trials (s)');
    legend([pha,phi],{'data-median','identical'},'Location','northoutside')
    exportgraphics(fh,fullfile(homedir,'..','plots','TCOM_corr_laser_on_off.pdf'),'ContentType','vector');
end
