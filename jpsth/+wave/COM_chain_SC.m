...
for fidx=30 % 1:numel(sums_conn_str)
    disp(fidx);
    if opt.strict
    ...
    onecom=wave.get_com_map('onepath',sums_conn_str(fidx).folder,'peak',opt.peak,'delay',opt.delay,'type',opt.type,'criteria',opt.criteria); %per su center of mass, normalized FR
