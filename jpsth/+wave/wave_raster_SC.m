...
if ~exist(fullfile(homedir,'..','sums_conn_str','var'))
    ...
    sums_conn_str=sums_conn_str(10:end,:);
    ...
    sumcon_sess=cellfun(@(x) ephys.path2sessid(x,'type',opt.type,'criteria',opt.criteria),{sums_conn_str.folder});
...
%     done=[done,sumcon_idx];
    ...
    if all(ismember(sig.mem_type(sig_idx,:),[1 2 5]))
        samp='s1';
        rsamp=4;
    elseif all(ismember(sig.mem_type(sig_idx,:),[3 4 6]))
        samp='s2';
        rsamp=8;
    else
        continue
...
%     end
    ...
    subplot(2,1,1)
    hold on
    psth=bz.raster(sumconn_currsess,sumcon_suids(sumcon_idx,:),rsamp,'type',opt.type,'criteria',opt.criteria);
%     ah=gca();
    xlim([2,4])
    subplot(2,2,4);
    hold on;
...
%     ylim([0,max(ylim())]);
    grid on
%     exportgraphics(fh,sprintf('wave_raster_SC_%d.png',sumcon_idx),'Resolution',300);
    if true
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('wave_raster_SC_%d.pdf',sumcon_idx)));
    end
%     close(fh);
%     waitfor(fh);
end
