...
[fcom.collection,fcom.com_meta]=wave.per_region_COM('stats_method','mean','selidx',opt.selidx,'delay',opt.delay,'type',opt.type,'criteria',opt.criteria);
ffrac.collection=ephys.per_region_fraction('memtype',opt.sust_type,'delay',opt.delay,'type',opt.type,'criteria',opt.criteria);
...
if opt.frac_COM
    ...
    if opt.export
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('per_region_TCOM_FRAC_%d.pdf',opt.delay)));
    end
...
if opt.COM_PVSST
    ...
    text(max(xlim()),max(ylim()),sprintf('r = %.3f, p = %.3f',r,p),'HorizontalAlignment','right','VerticalAlignment','top');
    if opt.export
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('per_region_COM_pv_sst_%d.pdf',opt.delay)));
    end
...
if opt.COM_sensemotor
    ...
    text(max(xlim()),max(ylim()),sprintf('r = %.3f, p = %.3f',r,p),'HorizontalAlignment','right','VerticalAlignment','top');
    if opt.export
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('per_region_COM_SMI_%d.pdf',opt.delay)));
    end
...
if opt.frac_PVSST
    ...
    if opt.export
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('per_region_frac_pv_sst_%d.pdf',opt.delay)));
    end
end
...
if opt.frac_sensemotor
    ...
    if strcmp(opt.sust_type,'sust')
        ...
        ylim([10,65]);
        set(gca(),'YTick',0:20:65,'XTick',-6:3:6);
    end
    xlabel('Sensory-motor transfer index')
    ylabel('Selective fraction')
    [r,p]=corr(coord(:,1),coord(:,2),'type',opt.corr);
    text(max(xlim()),max(ylim()),sprintf('r = %.3f, p = %.3f',r,p),'HorizontalAlignment','right','VerticalAlignment','bottom');
    if opt.export
        exportgraphics(fh,fullfile(homedir,'..','plots',sprintf('per_region_frac_sense_motor_%d.pdf',opt.delay)));
    end
end
end
