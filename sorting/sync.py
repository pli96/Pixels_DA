...
def parseLJWVDPAEventsV2(events):
    """
    if no cue history, assume sample
    if has cue history, check time diff
    time diff < 5: discard and report error
    5 < time diff < 10: this cue is test, last cue is sample
    time diff > 10
        if sample and test set
            if responsed
                add responsed trial
    ...
    for eidx in range(len(events)):
        cue = events[eidx][1] & 0x1C
        cueTS = events[eidx][0]

        if cue >= 4 and cueTS > lastTS + s1s and cueTS < lastTS + 5 * s1s:
            print("error processing evt idx ", eidx, cue, cueTS, lastCue, lastTS)
        elif cue >= 4 and cueTS > lastTS + 5 * s1s and cueTS < lastTS + 10 * s1s:
            print("catch", eidx, cueTS)
            test = cue
            testTS = cueTS
            sample = lastCue
            sampleTS = lastTS
            lastCue = cue
            lastTS = cueTS
        elif cue >= 4 and cueTS > lastTS + 10 * s1s and cueTS < lastTS + 19 * s1s:

            if sample > 0 and test > 0:
                print("successfully added evt idx ", eidx, cueTS)
                trials.append(
                    [
                        sampleTS,
                        testTS,
                ...
                print("miss component 1", eidx, cueTS, sample, test)
            lastCue = cue
            lastTS = cueTS

        elif cue >= 4 and cueTS > lastTS + 19 * s1s:

            if sample > 0 and test > 0:
                print("error gap too large, added evt idx ", eidx, cueTS)
                trials.append(
                    [
                        sampleTS,
                        testTS,
                ...
                print("miss component 2", eidx, cueTS, sample, test)

            lastCue = cue
            lastTS = cueTS

        elif cue >= 4 and cueTS < lastTS + 1.1 * s1s:
            print("skip", eidx, cueTS)
        else:
            print("others", eidx, cueTS)
...
def runsync(project):
    ...
    if project == 'VDPAP':
        trials = parseLJWVDPAEventsV2(events)
    elif project == 'dual_task':
