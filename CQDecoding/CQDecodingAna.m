...
%     disp(datetime)
    ...
    num_resample_runs=5;%define the decoding times
    ...
    for ExcludeTransientSustainedNeu=[3] % 0 for not exclude, 1 for excluding transient neurons, 2 for exclude sustained neurons,3 only with Transient neruons, 4 noly with Sustained neurons
        ...
        %Neuron number with specific sig. Bin based cluster-based permutation test
        %111  79   30   19   3
        %Transient neuï¼?33
        %Sustained neu: 3
        ...
        %disp(['TotalTrainingDay-' num2str(size(DayBasedPerformanceNeuron,1))])
        %for iTargetDay=1:size(DayBasedPerformanceNeuron,1)    
            %TrainingDayID=DayBasedPerformanceNeuron{iTargetDay,1};
            %TrainingDayID=regexprep(TrainingDayID,'  ','-','ignorecase');
            % extract the neurons in this specific training day
            %TargetNeuronID=DayBasedPerformanceNeuron{iTargetDay,4};

        %disp(['TotalMiceNum-' num2str(size(MiceBasedPerformance,1))])
        %for iTargetDay=1:size(MiceBasedPerformance,1)    
            %TrainingDayID=MiceBasedPerformance{iTargetDay,1};
            %TrainingDayID=regexprep(TrainingDayID,'  ','-','ignorecase');
            %% extract the neurons in this specific training day
            %TargetNeuronID=MiceBasedPerformance{iTargetDay,6};    

            if length(TargetNeuronID)>=0
            	%disp(['CurrentDayNum-' num2str(iTargetDay)])
                %num_neuron_ForDecoding=length(TargetNeuronID)-1;% if IsCorrectOrErrorOrAllTrials=4, NL-430; ChR-380; NpHR-340
                %disp([TrainingDayID '-' num2str(num_neuron_ForDecoding)])
                ...
                for IsLaser=1%:length(LaserPhase)
                    ...
                    if min(AddSustainedNeuronWithSigBinNum)>=0&&max(AddSustainedNeuronWithSigBinNum)<=5
                        ...
                        NonSelectiveNeuID=NeuronIndexWithDifferentSig1SecondBin{1};
                        TargetNeuronIDWithSpecificSigBin=[];
                        for i=1:length(AddSustainedNeuronWithSigBinNum)
                            TargetNeuronIDWithSpecificSigBin=[TargetNeuronIDWithSpecificSigBin;NeuronIndexWithDifferentSig1SecondBin{AddSustainedNeuronWithSigBinNum(i)+1}];
                        end
                        %TargetNeuronIDWithSpecificSigBin=setdiff(TargetNeuronIDWithSpecificSigBin,BothSwithNeuID_Pemu);
                        TargetNeuronIDWithSpecificSigBin=intersect(TargetNeuronIDWithSpecificSigBin,TransientNeuID);
